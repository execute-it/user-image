...
echo "${DARKGREY}[GIT: $(echo `git status` | grep "nothing to commit" > /dev/null 2>&1; if [ "$?" -ne "0" ]; then \
echo "\[\e[1;31m\]*\[\e[0m\]"; fi) ${DARKGREY}] "; fi )

	if [[ $EUID -ne 0 ]]; then
		PS1+="\[${ARRCOLOR}\]>\[${NOCOLOR}\]" # Normal user
	else
		PS1+="\[${ARRCOLOR}\]>\[${NOCOLOR}\]" # Root user
	fi
